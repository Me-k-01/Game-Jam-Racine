...
public class RootPlayer : MonoBehaviour {   
    ...
    void Start() {  
        ...
        topLeft = this.gameObject.transform.position - new Vector3(0, -gridSizeWidth/2, 0);
        GameObject newRoot = CreateRoot(topLeft);
        instances[gridSizeWidth/2, 0] = newRoot;
    }
    void Update() {
        if (Input.GetKeyDown(KeyCode.A)) {
            // TODO : i,j
            int i = gridSizeWidth/2;
            inc += 1;
            int j = inc;
